...
export function calculateMarketStatus(
  ...
  if (earningsGrowth < 0) {
    return {
      ...
      label: "景気悪化",
      description:
        "企業利益が減少しています。PERが維持されても株価には下押し圧力がかかります。",
    };
  }
  ...
  if (interestRate >= 6 && earningsGrowth < 5) {
    return {
      ...
      label: "金利逆風",
      description:
        "高い金利が株式の評価を抑えやすい環境です。利益成長が弱いと株価は伸びにくくなります。",
    };
  }
  ...
  if (
    ...
    return {
      ...
      label: "やや過熱",
      description:
        "金利が高い一方でPERは高めです。利益成長が追いつかないと、株価は期待の修正を受けやすくなります。",
    };
  }
  ...
  if (earningsGrowth >= 10 && interestRate <= 3 && gap < VALUATION_THRESHOLDS.somewhatExpensive) {
    return {
      ...
      label: "成長相場",
      description:
        "企業利益の伸びが強く、金利負担も小さいため、株価には追い風です。",
    };
  }
  ...
  return {
    ...
    label: "中立",
    description:
      "利益成長・金利・PERのバランスはおおむね落ち着いています。個別の変化が株価を左右しやすい局面です。",
  };
}
...
export function generateExplanation(
  ...
  if (earningsGrowth < 0) {
    parts.push(
      `企業利益は${Math.abs(earningsGrowth).toFixed(0)}%減少しています。EPSが下がると、同じPERでも株価は押し下げられます。`,
    );
  } else if (earningsGrowth >= 10) {
    parts.push(
      `利益成長率${earningsGrowth.toFixed(0)}%は高めです。株価上昇の土台は、企業そのものの利益拡大にあります。`,
    );
    ...
    parts.push(
      `利益成長は${earningsGrowth.toFixed(0)}%と控えめです。この環境ではPERの変化が株価に大きく効きます。`,
    );
  }

  if (gap >= 8) {
    parts.push(
      `金利${interestRate.toFixed(1)}%の環境でPER${peRatio.toFixed(0)}倍は高い水準です（参考PERは約${referencePER.toFixed(0)}倍）。株価の多くが投資家の高い期待に支えられています。`,
    );
  } else if (gap >= 3) {
    parts.push(
  ...
  if (inflationRate >= 5) {
    ...
    parts.push(
      `インフレ${inflationRate.toFixed(1)}%では、名目株価${Math.round(nominalStockPrice).toLocaleString("ja-JP")}に対し実質価値は約${Math.round(realStockPrice).toLocaleString("ja-JP")}（差約${Math.round(drag).toLocaleString("ja-JP")}）です。`,
    );
  ...
  if (parts.length === 1) {
    parts.push(
      `名目株価は約${Math.round(nominalStockPrice).toLocaleString("ja-JP")}、実質株価は約${Math.round(realStockPrice).toLocaleString("ja-JP")}です。スライダーを動かして因果関係を感じてみましょう。`,
    );
  }
...
export function generateLearningPoint(
  ...
  if (gap >= 8) {
    return "株価上昇 ＝ 企業が強くなった、とは限らない。";
  }
  if (interestRate >= 6) {
    return "金利が上がると、将来利益の現在価値は低下しやすい。";
  }
  if (earningsGrowth < 0) {
    return "利益が減れば、PERが変わらなくても株価は下がる。";
  }
  if (earningsGrowth >= 10 && gap < 3) {
    return "強い利益成長は、株価上昇のいちばん健全なエンジンになる。";
  }
  if (gap <= -5) {
    return "同じ利益でも、市場の期待が低いと株価は抑えられる。";
  }
  return "株価は「利益」と「期待（PER）」の掛け算で決まる。";
}
